...
export function ServiceForm({ service }: ServiceFormProps) {
  ...
  const form = useForm<ServiceFormValues>({
    ...
    defaultValues: service ? {
      ...
      testimonial: { quote: "", name: "", role: "", avatar: "https://placehold.co/100x100.png" },
      gallery: [],
      industries: ""
    },
  });
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <div className="flex items-center justify-between">
            <h1 className="text-3xl font-bold">{service ? "Edit Service" : "Create New Service"}</h1>
          ...
          <CardContent className="space-y-4">
            {offeringsFields.map((field, index) => (
              <div key={field.id} className="flex gap-4 items-start p-4 border rounded-md relative">
                ...
                <Button type="button" variant="destructive" size="icon" onClick={() => removeOffering(index)}>
                  <Trash2 className="h-4 w-4" />
                </Button>
              </div>
            ))}
            <Button type="button" variant="outline" onClick={() => appendOffering({ title: "", description: "" })}>
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                <FormField control={form.control} name="techStack.frontend" render={({ field }) => (
                    <FormItem><FormLabel>Frontend</FormLabel><FormControl><Input {...field} placeholder="React, Next.js, TypeScript" /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="techStack.backend" render={({ field }) => (
                    <FormItem><FormLabel>Backend</FormLabel><FormControl><Input {...field} placeholder="Node.js, Python, Django" /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="techStack.database" render={({ field }) => (
                    <FormItem><FormLabel>Database</FormLabel><FormControl><Input {...field} placeholder="PostgreSQL, MongoDB" /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="techStack.tools" render={({ field }) => (
                    <FormItem><FormLabel>Tools</FormLabel><FormControl><Input {...field} placeholder="Docker, Git, Webpack" /></FormControl><FormMessage /></FormItem>
                )} />
            </CardContent>
        </Card>
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                ...
                <FormField control={form.control} name="testimonial.name" render={({ field }) => (
                    <FormItem><FormLabel>Client Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="testimonial.role" render={({ field }) => (
                    <FormItem><FormLabel>Client Role</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="testimonial.avatar" render={({ field }) => (
                    <FormItem><FormLabel>Client Avatar URL</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )} />
            </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
                ...
                <div className="grid gap-2 flex-1">
                    ...
                    <FormField control={form.control} name={`gallery.${index}.alt`} render={({ field }) => (
                     ...
                     <FormField control={form.control} name={`gallery.${index}.dataAiHint`} render={({ field }) => (
                        <FormItem><FormLabel>AI Hint</FormLabel><FormControl><Input {...field} placeholder="e.g. dashboard analytics" /></FormControl><FormMessage /></FormItem>
                    )} />
                </div>
            ...
            <Button type="button" variant="outline" onClick={() => appendGallery({ src: "https://placehold.co/600x400.png", alt: "", dataAiHint: "" })}>
        ...
        <Card>
            ...
            <CardContent>
                <FormField control={form.control} name="industries" render={({ field }) => (
                    <FormItem><FormLabel>Industries</FormLabel><FormControl><Input {...field} placeholder="E-commerce, Healthcare, Education" /></FormControl><FormMessage /></FormItem>
                )} />
            </CardContent>
        </Card>
      </form>
