...
const formSchema = z.object({
  ...
  id: z.string().min(3, "Slug/ID must be at least 3 characters.").refine(s => !s.includes(' '), "Slug cannot contain spaces."),
...
export function ProductForm({ product }: ProductFormProps) {
  ...
  const form = useForm<ProductFormValues>({
    ...
    defaultValues: product ? {
      ...product,
      features: product.features.map(f => ({ title: f.title, description: f.description }))
    } : {
      name: "",
      id: "",
      tagline: "",
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <div className="flex items-center justify-between">
            <h1 className="text-3xl font-bold">{product ? "Edit Product" : "Create New Product"}</h1>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <FormField control={form.control} name="id" render={({ field }) => (
              <FormItem><FormLabel>Slug / ID</FormLabel><FormControl><Input {...field} placeholder="e.g., paarsh-crm" disabled={!!product} /></FormControl><FormMessage /></FormItem>
            )} />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Detail the key features of this product. Icons will be assigned automatically.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
              ...
              <div key={field.id} className="flex gap-4 items-start p-4 border rounded-md relative">
                <div className="grid gap-2 flex-1">
                    ...
                    <FormField control={form.control} name={`features.${index}.description`} render={({ field }) => (
                        <FormItem><FormLabel>Feature Description</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                </div>
            ...
            <Button type="button" variant="outline" onClick={() => appendFeature({ title: "", description: "" })}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Showcase images of the product interface.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button type="button" variant="outline" onClick={() => appendGallery({ src: "https://placehold.co/800x600.png", alt: "", hint: "" })}>
