...
export function SuccessStoryFormModal({ isOpen, onOpenChange, onSave, story }: Props) {
  ...
  useEffect(() => {
    if (story) {
      ...
      setValue("role", story.role);
      setValue("quote", story.quote);
      setValue("published", story.published);
      setPreviewImage(story.avatar);
      setAvatarBase64("");
    } else {
      reset();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{story ? "Edit Success Story" : "Add New Success Story"}</DialogTitle>
          <DialogDescription>
            {story ? "Update the success story details." : "Fill in the details to add a new success story."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="role" {...register("role")} placeholder="e.g., Software Engineer at TCS" />
            {errors.role && <p className="text-sm text-red-500">{errors.role.message}</p>}
          </div>

          <div className="space-y-2">
            <Label htmlFor="quote">Success Story Text *</Label>
            <Textarea id="quote" {...register("quote")} placeholder="Enter the success story quote..." rows={4} />
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? "Saving..." : story ? "Update" : "Add"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
