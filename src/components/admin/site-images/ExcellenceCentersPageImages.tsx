...
interface ExcellenceCentersPageImagesProps {
  images: SiteImage[];
}

export function ExcellenceCentersPageImages({ images }: ExcellenceCentersPageImagesProps) {
  ...
  const handleSave = async (data: any) => {
    try {
      if (selectedImage) {
        ...
        await addImage({ page: 'excellence-centers', ...data }).unwrap();
        toast({ title: "Image Added", description: "The new image has been added to the excellence centers page." });
      }
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Excellence Centers Page Images</CardTitle>
            <CardDescription>Manage the images displayed on the public excellence centers page.</CardDescription>
          </div>
        ...
        <CardContent>
          ...
          {images.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              <p>No images found for the excellence centers page.</p>
