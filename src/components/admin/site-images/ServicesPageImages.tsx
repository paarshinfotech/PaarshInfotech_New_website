...
interface ServicesPageImagesProps {
  images: SiteImage[];
}

export function ServicesPageImages({ images }: ServicesPageImagesProps) {
  ...
  const handleSave = async (data: any) => {
    try {
      if (selectedImage) {
        ...
        await addImage({ page: 'services', ...data }).unwrap();
        toast({ title: "Image Added", description: "The new image has been added to the services page." });
      }
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Services Page Images</CardTitle>
            <CardDescription>Manage the images displayed on the public services pages.</CardDescription>
          </div>
        ...
        <CardContent>
          ...
          {images.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              <p>No images found for the services page.</p>
