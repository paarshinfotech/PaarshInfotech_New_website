...
interface MediaPageImagesProps {
  images: SiteImage[];
}

export function MediaPageImages({ images }: MediaPageImagesProps) {
  ...
  const handleSave = async (data: any) => {
    try {
      if (selectedImage) {
        ...
        await addImage({ page: 'media', ...data }).unwrap();
        toast({ title: "Image Added", description: "The new image has been added to the media page." });
      }
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Media Page Images</CardTitle>
            <CardDescription>Manage the images displayed on the public media page.</CardDescription>
          </div>
        ...
        <CardContent>
          ...
          {images.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              <p>No images found for the media page.</p>
